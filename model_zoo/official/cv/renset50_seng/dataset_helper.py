    ...
    def __init__(self, dataset, dataset_sink_mode=True, sink_size=-1, epoch_num=1, iter_first_order=1):
        ...
        if dataset_sink_mode:
            if context.get_context("device_target") == "Ascend":
                self.iter = _DatasetIterMSLoopSink(dataset, sink_size, epoch_num, iter_first_order)
            elif context.get_context("device_target") == "GPU":
                self.iter = _DatasetIterMS(dataset, sink_size, epoch_num)
...
class _DatasetIter:
    ...
    def __init__(self, dataset, sink_size, epoch_num):
        ...
        if not hasattr(dataset, '__transfer_dataset__'):
            ...
            if not hasattr(dataset, '__no_send__'): #__no_send__ is a key used by xudong (also in model.py)
                """Engine dataset to write data to tdt queue."""
                if not hasattr(dataset, '__has_sent__'):
                    dataset.__transfer_dataset__.send(epoch_num)
