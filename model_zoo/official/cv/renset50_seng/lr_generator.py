...
def _generate_steps_lr(lr_init, lr_max, total_steps, warmup_steps):
    ...
    """
    decay_epoch_index = [0.34 * total_steps, 0.66 * total_steps, 0.95 * total_steps]
    lr_each_step = []
...
def _generate_exp_lr(lr_baseline, decay, total_epochs, steps_per_epoch, warmup_steps, decay_epochs):
    """
    Exponential learning rate
    Note that the params are passed in different order
    In the application, pass --lr_init as basic lr, --lr_end as decay, --lr_max as total_epochs
    """
    lr_each_step = []
    ...
    for i in range(total_steps):
        if i < warmup_steps:
            real_lr_init = lr_baseline * 0.1
            lr_inc = (lr_baseline - real_lr_init) / warmup_steps
            lr_local = real_lr_init + lr_inc * (i + 1)
        else:
            epoch = (i + 1 - warmup_steps) / steps_per_epoch
            base = (1.0 - epoch / total_epochs)**decay
            lr_local = lr_baseline * base
            if epoch >= decay_epochs:
                lr_local = lr_local * 0.1
            # if epoch >= decay_epochs + 1:
            #     lr_local = lr_local * 1.0
        lr_each_step.append(lr_local)
    lr_each_step = np.array(lr_each_step).astype(np.float32)
    return lr_each_step

def get_lr(lr_init, lr_end, lr_max, warmup_epochs, total_epochs, steps_per_epoch, lr_decay_mode, decay_epochs):
    """
    ...
    elif lr_decay_mode == 'exp':
        # decay_epochs is set to total_epochs. This disables decay in the final stage.
        lr_each_step = _generate_exp_lr(lr_init, lr_end, lr_max, steps_per_epoch, warmup_steps, decay_epochs)
    else:
